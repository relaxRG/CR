...
- [ ] EAS Build 57 触发（代码已完成，需在 Cloud PC 上准备 ~/.apple-creds/ 凭证文件 + EXPO_TOKEN 后执行 bash scripts/trigger-eas-build.sh --submit）
...
- [ ] 触发 EAS Build 57 并提交 TestFlight（凭证文件就位后执行 bash scripts/trigger-eas-build.sh --submit）
...
- [ ] 门店在售清单升级（引用三模块数据）
- [ ] 门店采购清单升级（供应商+自购链接）
- [x] 闪退诊断：增加根级错误边界、全局 JavaScript 异常与未处理 Promise 拒绝捕获。
- [x] 闪退诊断：实现脱敏的本地环形日志，记录启动、导航、AppState、全局异常和错误堆栈。
- [x] 闪退诊断：提供受控的屏幕诊断面板及日志导出/复制路径，供真机复现后回传完整日志。
- [x] 闪退诊断：为全局捕获与日志缓冲补充回归测试，并验证错误不会阻断应用错误边界的恢复操作。
- [ ] 严格 iOS 排障：确认可用的真机或模拟器、Xcode、符号文件和当前构建号，稳定复现相同触发路径。
- [ ] 严格 iOS 排障：采集并归档完整 Xcode 控制台、`.ips` Crash Report、线程调用栈和异常类型；若不可取得，提供准确的 Xcode 导出指引。
- [ ] 严格 iOS 排障：逐项检查权限说明、原生崩溃信号、数组/空值访问、主线程 UI 更新、生命周期、权限拒绝、存储迁移与依赖/架构兼容性。
- [ ] 严格 iOS 排障：仅根据已验证根因实施最小修复，并在 Debug/Release、模拟器/真机复测同一触发路径。
- [ ] 严格 iOS 排障：输出含根因、修改、验证结果与剩余风险的排障报告。
- [ ] 全权 iOS 排障：盘点现有授权下可用的 macOS、Xcode、iOS Simulator 与云端 iOS 测试环境，不要求用户操作。
- [ ] 全权 iOS 排障：自行在可用模拟器或云端环境构建、启动和采集原生日志；覆盖可能的闪退路径。
- [ ] 全权 iOS 排障：仅根据可验证证据实施最小修复，并在相同模拟器路径连续运行至少五次无闪退后才标记模拟器验证通过。
- [ ] 全权 iOS 排障：输出根因、文件级改动、模拟器/云端测试结果和仍无法被模拟器覆盖的真机风险。
- [ ] 云端 iOS 根因排障：修复 macOS 测试工作流的 pnpm 版本冲突，并确认模拟器/云端作业实际可启动。
- [ ] 云端 iOS 根因排障：在云端 iOS 环境采集启动和高风险路径的异常、控制台与调用栈，区分测试环境故障和 App 闪退根因。
- [ ] 云端 iOS 根因排障：依据已验证根因进行最小修复，并对同一路径执行至少五次连续无闪退运行。
- [ ] iOS 验证续办：在修正后的进程检测下，于同一云端 iOS 模拟器完成五次连续 Release 冷启动并保存每次原生日志结论。
- [ ] iOS 验证续办：按用户报告的易闪退操作路径重复测试，记录是否真实复现、异常类型与调用栈，或明确仍缺少的复现证据。
- [x] iOS 收尾：对当前云端 macOS 作业设置 10 分钟运行器分配上限；超时后停止轮询且不重复提交同一工作流。
- [ ] iOS 收尾：云端不可用时完成单元测试、类型检查、lint、依赖兼容、iOS 预构建/Pod 集成、Web 启动与高风险路径冒烟测试。
- [x] iOS 收尾：复核诊断能力不会吞掉异常、无限重试、泄露凭据或改变原有崩溃语义；区分已排除的 JS 风险与剩余原生风险。
- [x] iOS 收尾：已完成全量 JS/业务回归、Expo 依赖诊断、iOS 预构建、冷启动脚本语法及诊断语义测试；CocoaPods 和 Web 运行态仍需可用的 macOS/iOS 或 Web 打包环境补充验证。
- [x] 测试摘要：按核心模块归类本轮全量单元测试和定向业务逻辑测试的通过结果、覆盖范围与未覆盖边界。
- [x] 原生风险报告：审查 Keychain/SecureStore、权限请求与本地存储代码，列出潜在 iOS 原生崩溃隐患及证据等级。
- [x] 原生风险报告：列出仅可由真实 iPhone 验证的风险点，并形成按操作、预期与日志证据组织的排查清单。
- [x] 原生风险报告：输出已通过的 JavaScript/业务逻辑定向测试详细结果与核心模块通过率。
- [x] 真机排查说明：输出 Keychain、权限和本地存储的具体原生风险、测试状态、真机操作与日志收集步骤。
- [x] 原生依赖审查：核对 Expo/React Native 原生模块、第三方库版本和已知 iOS 兼容或内存风险。
- [x] 启动桥接审查：检查异步初始化、未处理 Promise、错误边界与原生桥接崩溃之间的覆盖边界。
- [x] 启动桥接修复：为同步启动中非阻塞的设备平台刷新补充拒绝处理，防止网络或 SecureStore 异常形成未处理 Promise。
- [x] 改动影响审查：汇总已修改文件与具体 diff，评估其对降低真实 iPhone 闪退风险的有效性和限制。
- [x] 原生静态加固：清点所有原生模块调用，找出可通过代码与配置静态排除或修复的隐患。
