...
export function InventoryCategoryManager({ visible, colors, categories, itemCounts, onUpsert, onMove, onSafeRemove, onClose }: Props) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    ...
    <View style={[S.sheet, { backgroundColor: colors.background, paddingBottom: Math.max(insets.bottom, 16) }]}>
      ...
      <View style={S.header}>
        <TouchableOpacity onPress={onClose} hitSlop={10} testID="inventory-category-manager-cancel"><Text style={{ color: colors.muted, fontWeight: "600" }}>取消</Text></TouchableOpacity>
        <Text style={[S.title, { color: colors.foreground }]}>进销存分类</Text>
        <TouchableOpacity onPress={onClose} hitSlop={10} testID="inventory-category-manager-save"><Text style={{ color: colors.primary, fontWeight: "700" }}>保存</Text></TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={S.list}>
        {categories.map((category, index) => {
          ...
          return <View key={category.id} style={[S.item, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            {!category.builtin ? <TouchableOpacity onPress={() => requestDelete(category)} style={S.textAction} testID={`inventory-category-delete-${category.id}`}><Text style={{ color: colors.destructive ?? "#DC2626" }}>删除</Text></TouchableOpacity> : <Text style={{ color: colors.muted, fontSize: 10 }}>内置</Text>}
          </View>;
        })}
      </ScrollView>
    ...
    <Modal visible={Boolean(editing)} transparent animationType="fade" onRequestClose={() => setEditing(null)}>
      <View style={S.center}>
        <View style={[S.dialog, { backgroundColor: colors.background }]}>
          ...
          <View style={S.dialogActions}><TouchableOpacity onPress={() => setEditing(null)}><Text style={{ color: colors.muted }}>取消</Text></TouchableOpacity><TouchableOpacity onPress={saveEdit}><Text style={{ color: colors.primary, fontWeight: "700" }}>保存</Text></TouchableOpacity></View>
        </View>
      </View>
    </Modal>
