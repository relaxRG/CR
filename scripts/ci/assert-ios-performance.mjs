...
const [candidatePath, baselinePath, configPath = "scripts/ci/ios-performance-thresholds.json"] = process.argv.slice(2);
if (!candidatePath || !baselinePath) {
  console.error("Usage: node scripts/ci/assert-ios-performance.mjs <candidate.json> <baseline.json> [thresholds.json]");
  process.exit(2);
}
...
const comparableDevice = (a, b) => {
  const aDevice = a?.device ?? {};
  const bDevice = b?.device ?? {};
  return aDevice.model === bDevice.model && aDevice.osMajor === bDevice.osMajor && aDevice.perfMode === bDevice.perfMode;
};
...
if (config.requireComparableDevice && !comparableDevice(candidate, baseline)) {
  failures.push("候选构建与基线不在同一设备型号、iOS 大版本或性能模式，拒绝比较 P95 回归");
}
