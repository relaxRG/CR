...
| `lib/integrations/meituan/bill-import-window.ts` | 最近一年导入计划器 | 应用层保留 12 个自然月；官方可查询日期按 7 天批次分片；窗口外明确走文件导入 |
...
美团官方日账单接口 `bill/list/yuan` 返回账单结算、菜品金额、优惠、平台费和 SKU 明细。**Cocktail R 已把自己的导入保留窗口设为最近 12 个自然月。**但官方文档仍限制 API 仅能查询近 90 天、单次日期跨度不得超过 7 天、前一日账单在当天 10 点前不可查询，并要求商家为应用申请账单调用权限。[2] 因此计划器会将最近一年中官方可查的日期拆成 7 天 API 批次，较早日期标记为“从美团管家导出文件导入”；不伪造或绕过官方查询边界。
...
`bill-import-window.ts` 将最近 12 个自然月拆分为两类任务：仍在官方近 90 天窗口内的日期按每批不超过 7 天生成 `meituan-openapi` 批次；更早日期生成 `file-import` 批次，提示从美团管家导出文件。`daily-bill-resume.ts` 为每个 API 批次建立 `门店ID + 月份 + 起始日 + 结束日` 的不可变断点键。批次只有从 `running` 显式写入成功结果后才是 `completed`；中断、网络失败和应用重启后，仅重新读取 `failed` 或 `pending` API 批次，已完成批次不会重复读取。文件导入批次绝不被标记为自动同步完成。
...
| 材料 | 建议内容 | 负责人/来源 |
|---|---|---|
| 企业主体证明 | 营业执照副本、统一社会信用代码、法定代表人或授权经办人信息 | 当前门店所属企业 |
| 门店归属证明 | 门店名称、稳定门店 ID、品牌/商户归属关系，以及当前门店对数据读取的授权声明 | 门店管理员/品牌主体 |
| 官方应用资料 | 在美团开发者平台创建的 `appid`、`appname`、应用简介、测试与生产环境配置 | 开发者账号 |
| 账单权限申请函 | 申请接口名称、只读权限范围、读取频率、账单数据字段、预计调用量、仅用于内部对账的用途说明 | 应用负责人 |
| 商家公司授权证明 | 商家盖章/电子授权，明确授权应用读取当前门店账单、订单及菜品明细 | 商户主体 |
| 技术方案 | OAuth 授权流程、服务端令牌保存、每日 7 天分批补拉、12 个月导入状态、退款处理和断点续传 | Cocktail R 技术方案 |
| 安全与隐私说明 | 不存账号密码、TLS 传输、令牌加密、最小权限、访问日志脱敏、30 天审计日志、撤销授权与删除流程 | 技术与数据负责人 |
| 对账验证记录 | 最近 30 天人工对账表：营业收入、商家优惠、平台优惠、平台费、结算额和差额处置 | 财务/门店管理员 |

自动对账应在安全服务端运行，而不是在移动 App 内保存密钥。应用层将显示最近 12 个月的导入状态；第一阶段建议每天一次，按官方日账单的 7 天查询窗口切片补拉；超过官方可查窗口的月份保留为文件导入快照。不得将分钟级轮询作为默认设计。若官方支持订单推送，则应在确认相应权限和签名验证方式后再接入事件驱动同步。[2]

## 授权令牌、浏览器登录与持续读取边界

美团开放平台公开 OAuth 文档说明，`Access Token` 有效期为 **2 小时**；对应 `Refresh Token` 有效期为 **30 天**，刷新会返回新的 Access Token 和 Refresh Token。[5] 因此，官方授权不是“永远保持登录”：服务端应在每次同步前检查授权关系，在 Access Token 临近到期时刷新；若 Refresh Token 到期、商家撤销授权、应用权限被收回或风控要求重新验证，则必须由用户重新完成官方授权。该有效期适用于美团开放平台 OAuth 文档所述模式；账单接口的实际业务权限仍受应用权限和商家授权证明控制。[2] [5]

浏览器会话也不应被视为永久登录。它会受到美团的主动退出、设备/异地风控、cookie 失效、验证码和账号安全策略影响。浏览器仅可作为人工导出或首次官方授权的辅助，不能作为无人值守同步的凭据来源。

## 最少操作的安全接入流程

推荐采用“**一次官方授权 + 后台自动续期 + 异常时才提醒**”的模式。首次仅由你在美团官方授权页完成一次登录、扫码或验证码确认；Cocktail R 服务端保存加密后的 OAuth 令牌并在到期前刷新。以后每日对账由后台按 7 天批次和断点状态自动完成，App 只显示“已同步到某日 / 有一批失败待重试 / 需要重新授权”。只有 Refresh Token 到期、商家撤销授权或美团要求重新验证时，才出现一次“重新连接美团官方账户”的按钮。全年早于官方窗口的月份集中保留为文件导入快照，不要求重复登录或每日手工导出。
...
[5]: [美团开放平台：根据 Code 请求 Access Token](https://open.meituan.com/docs/detail?id=2)；[刷新 Access Token](https://open.meituan.com/docs/detail?id=4)
