# 美团管家智能版当前单店导入 MVP 与官方对账接入审计

## 目标与范围

本次实现为 Cocktail R 新增**美团管家智能版当前单店数据导入引擎**，优先覆盖“月度营业收入”和“菜品大类”，并提供美团官方日账单的可审计映射与防串联校验。当前阶段只生成**待确认预览**，不在未经用户确认的情况下写入现有月报、账户、库存或薪资状态。
...
| 文件 | 作用 | 关键保证 |
|---|---|---|
| `lib/integrations/meituan/monthly-import.ts` | 月度营业收入、菜品大类归并和差额校验 | 已绑定当前门店与月份作为硬约束；`Food`/`food`/全角变体合并；不静默平差 |
| `lib/integrations/meituan/excel-adapter.ts` | Excel 多工作表表头识别 | 必须读取原始门店 ID 并匹配当前绑定，禁止仅按门店名称猜测；支持说明行后的表头定位 |
| `lib/integrations/meituan/daily-bill-import.ts` | 官方日账单字段映射与幂等校验 | `source + 当前绑定门店ID + orderId` 幂等键；其他门店、跨月、退款和金额冲突阻断 |
...
当前只使用一家门店，导入入口会固定到一个已配置的 `MeituanSingleStoreBinding`，不提供选店控件。现有 `MonthlyReportStore` 以 `rawMonth` 单键替换同月快照，在当前单店下可在用户确认后使用；但未来若启用第二家门店，**不能直接复用该单键存储**，必须先把唯一键升级为：
...
当前写入流程应为：原始导入文件/日账单响应不可变归档 → 标准化预览 → 验证当前绑定门店与月份 → 用户确认 → 生成带 `source`、`storeId`、`month`、`importVersion` 的单店快照。只有未来确实启用第二家门店时，才实施上述多店唯一键迁移与集团汇总，避免过早引入不需要的复杂度。
...
1. 由用户提供一份脱敏的智能版当前门店“月度营业收入”和“菜品销售统计—大类”导出样本，确认真实表头别名并完成门店 ID 绑定。
2. 将本次解析器接入“报表 → 导入美团经营数据”的预览页面，但暂不写入 `MonthlyReportStore`。
3. 完成单店导入预览和人工确认 UI 后，开放确认写入当前月报。
4. 使用当前门店、一个完整自然月进行人工对账；通过后再决定是否启用官方日更。
