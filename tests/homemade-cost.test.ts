...
const mkPrep = (id: string, name: string, nameAlt: string, ingredients: string[] = [], yieldStr = ""): HomemadePrep => ({
...
const honeyPrep = mkPrep("p1", "蜂蜜糖浆", "Honey Syrup", ["100ml 水", "100g 蜂蜜"], "200ml");

describe("estimatePrepCost — homemadeId 字段", () => {
  it("无自制库时所有 item.homemadeId 为 null", () => {
    const prep = mkPrep("px", "测试自制品", "Test Prep", ["30ml 伏特加", "20ml 水"]);
  ...
  it("原料匹配到自制库时 homemadeId 有值", () => {
    const prep = mkPrep("px", "测试自制品", "Test Prep", ["30ml 蜂蜜糖浆"]);
  ...
  it("自循环防护：自制品不匹配到自身", () => {
    ...
    const selfRef = mkPrep("p1", "蜂蜜糖浆", "Honey Syrup", ["50ml 蜂蜜糖浆"]);
  ...
  it("bottleId 和 homemadeId 互斥（不同时有值）", () => {
    const prep = mkPrep("px", "测试", "Test", ["30ml 伏特加", "20ml 蜂蜜糖浆"]);
...
describe("estimatePrepCost — 基础功能不受影响", () => {
  ...
  it("无法估算的原料 cost 为 null", () => {
    const prep = mkPrep("px", "测试", "Test", ["一些神秘原料"]);
    const cost = estimatePrepCost(prep, [], []);
